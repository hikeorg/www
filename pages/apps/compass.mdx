...
```yaml
rules:
  - name: dangerous sql files
    match:
      - "**/*.sql"
      - "database/**/*.*"
    comment: |-
      Be careful editing this files!
  - name: commit lockfiles
    check: true
    skip: true
    match: ["package.json", "!yarn.lock"]
  - name: best practices graphql
    match:
      - "**/*.gql"
    comment: |-
      :warning:

      When you update graphql schemas remember:

      **When deleting fields**
      - [ ] Mark fields as deprecated
      - [ ] Delete fields after grace period
